...
class ScenarioManager:
    ...
    def create_edge_manager_from_scenario_runner(self, application,
        ...
        for e, edge in enumerate(
                self.scenario_params['scenario']['edge_list']):
            edge_manager = EdgeManager(self.world, edge, self.cav_world, carla_client=self.client, world_dt=world_dt,
                                       edge_dt=edge_dt, search_dt=search_dt, mode=config_yaml['edge_base']['mode'], other_vehicles=other_vehicles, distributed=self.run_distributed)
            if 'rsus' in edge:
                for index, cav in enumerate(edge['rsus']):
                    rsu_manager = RSUManager(self.world, cav,
                                       self.carla_map,
