...
try:
    from opencda.core.prediction.prediction.r2p2.r2p2_model import R2P2
except ImportError:
...
class R2P2PredictorOperator(erdos.Operator):
    ...
    def predict(self, point_cloud, tracks):
        self._logger.debug('R2P2PredictorOperator: starting prediction')
        start_time = time.time()

        nearby_vehicle_trajectories, nearby_vehicle_ego_transforms, \
            nearby_trajectories_tensor, binned_lidars_tensor = \
        self._preprocess_input(tracks, point_cloud)

        num_predictions = len(nearby_vehicle_trajectories)

        z = torch.tensor(
            np.random.normal(size=(num_predictions,
                                   self.prediction_num_future_steps,
