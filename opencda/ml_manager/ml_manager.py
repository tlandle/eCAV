...
class MLManager(object):
    ...
    def _init_distributed(self):
        ...
        # Service endpoints
        self.yolo_endpoint = self.config.get('yolo_endpoint', 'http://localhost:8000')
        self.bm2cp_vehicle_endpoint = self.config.get('bm2cp_vehicle_endpoint', 'http://localhost:8001')
        self.bm2cp_edge_endpoint = self.config.get('bm2cp_edge_endpoint', 'http://localhost:8002')
        
        print(f"[ML Manager] YOLO endpoint: {self.yolo_endpoint}")
        print(f"[ML Manager] BM2CP Vehicle endpoint: {self.bm2cp_vehicle_endpoint}")
        print(f"[ML Manager] BM2CP Edge endpoint: {self.bm2cp_edge_endpoint}")
    ...
    def _detect_yolo_distributed(self, rgb_images):
        """Run YOLO via distributed service"""
        request_data = {
            'images': [img.tolist() if isinstance(img, np.ndarray) else img 
                      for img in rgb_images]
        }
        
        try:
            response = requests.post(
                f"{self.yolo_endpoint}/predict",
                json=request_data,
                timeout=10
            )
            ...
            # Convert response to match local YOLO output format
            return self._parse_yolo_response(response.json())
    ...
    def draw_2d_box(self, result, rgb_image, index):
        ...
        for i in range(bounding_box.shape[0]):
            ...
            label_name = names[label] if names else str(label)
    ...
    def _parse_yolo_response(self, response: Dict):
        """Parse distributed YOLO response to match local format"""
        # Create a mock YOLO result object that matches the expected interface
        class YOLOResult:
            def __init__(self, detections):
                self.xyxy = []
                self.names = detections[0]['names'] if detections else {}
                
                for det in detections:
                    boxes = torch.tensor(det['boxes'])
                    self.xyxy.append(boxes)
        
        return YOLOResult(response['detections'])
