...
- **Scenario B COMPLETE (2026-07-27, run 12; commits 6eef5c89 + 6cefda1a):** All Phase 1 Scenario B requirements met.
  - Vehicle handoff: tick=63, 986 bytes, warm KF transfer ✓
  - Ambulance detected via edge predictions at 42.3m (before local sensor range) ✓
  - Ego waits for NPC, merges right, no crash in 700 ticks ✓
  - **PREDICTIVE OBSTACLE HANDOFF tick=161**, NPC at x=98.3 (16.7m before locale boundary), 986 bytes ✓
  - **Advance-warning window = 82 ticks (4.1s)** before RSU1 first detected NPC ✓
  - Three bugs fixed to get here: (1) `if not beacons:` guard silently dropped all RSU detections post ego-handoff; (2) KeyError at handoff boundary (VM in list but not in beacons snapshot); (3) `_find_obstacle_kf` used `kf.x[1]` (CARLA_z height ≈ 0) instead of `kf.x[2]` (CARLA_y lateral ≈ 140) — made NPC-to-track distance always ≈ 140m, fallback always returned None.
  - **OPEN:** `[TRACK-DBG]` / `[EGO-DBG]` / `[SCENB-DBG]` diagnostics still at WARNING level — remove before Phase 2.
  - **OPEN:** Curved-road suppression bug `behavior_agent.py:1555` documented in plan (Tyler scope).
  - **Next:** Phase 2 `-eo` distribution plan (Step 8); separate plan needed.
