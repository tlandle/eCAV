## Release History of eCAV
This page demonstrates all the changes since the origin release v0.1.0 with more detailed imags.

--- 
### v0.1.2
##### Map manager
eCAV now adds a new component `map_manager` for each cav. It will dynamically load road topology, traffic light information, and dynamic
...
eCAV now supports data dumping simultaneously for multiple CAVs to develop V2V perception 
...
vehicle for trajectory prediction purpose. Run `python root_of_ecav/scripts/generate_prediction_yaml.py`
...
eCAV now supports both CARLA 0.9.12 and 0.9.11. Users needs to set CARLA_VERSION variable before
installing eCAV. When users run ecav.py, -v argument is required to classify the CARLA version for
eCAV to select the correct API.
