...
eCAV provides benchmark scenarios that researchers can use directly without any modification. We will also be adding new testing scenarios in future releases. Running these 
scenario testings only need one line of command:
```she
cd ~/eCAV
python ecav.py -t scenario_name -v carla_version --apply_ml --record
```
Parameters explanation:
* `-t`: The name of the tesitng scenario. A python script with the same name should exist in
`ecav/scenario_testing/` to construct the simulation, and a yaml file with the same name should exist in 
`ecav/scenario_testing/config_yaml/` to define the simulation parameters.
...
Below we will demonstrate some examples of running the benchmark testings in eCAV.
...
```sh
python ecav.py -t single_2lanefree_carla -v 0.9.11
```
...
```sh
python ecav.py -t single_town06_carla  -v 0.9.11 --apply_ml
```
...
```sh
python ecav.py -t single_town06_cosim  -v 0.9.11 --apply_ml
```
...
```sh
python ecav.py -t platoon_stability_2lanefree_carla -v 0.9.11
```
...
```sh
python ecav.py -t platoon_joining_2lanefree_carla -v 0.9.11
```
...
```sh
python ecav.py -t platoon_joining_2lanefree_cosim -v 0.9.11
```
![](images/platoon_joining_cosim.gif)

#### 4. Platoon back-join (Pytorch required)
```sh
python ecav.py -t platoon_joining_town06_carla  -v 0.9.11 --apply_ml
```
