...
* [__Local Installation__](#local-installation)
    ...
    * [__2. Install eCAV__](#ecav-installation)
...
* __System requirements.__ Any 64-bits OS should run eCAV. We highly recommends Ubuntu  16.04/18.04/20.04.
...
---
## Local Installation
To get eCAV v0.1 running with complete functionality, you will need four things: CARLA, eCAV, and
Pytorch (optional). Pytorch is required only when you want to activate the perception module; otherwise eCAV
will retrieve all perception information from the simulation server directly.

###  1. CARLA Installation (>= 0.9.11 required)

There are two different recommended ways to install the CARLA simulator and either way is fine for using eCAV. <br>
Note: If you want to use the customized highway map with full assets (.fbx, .xml and .xodr) in eCAV, 
...
eCAV is tested both at 0.9.11 and 0.9.12.
...
For advanced CARLA usage that involves extensive customizations, [Build CARLA from Source](https://carla.readthedocs.io/en/0.9.11/build_linux/) is also supported by eCAV. Though source build in 
Windows OS is supported by CARLA, Ubuntu is the preferred OS as the eCAV was developoed in Ubuntu 18.04.  

<strong>Note: eCAV do not require CARLA source build. However, customized map with building/lane/traffic light/road surface materials assets  in CARLA  require source build. 
...
### 2. eCAV Installation
First, download eCAV github to your local folder if you haven't done it yet.
```sh
git clone https://github.com/ucla-mobility/eCAV.git
cd eCAV
```
Make sure you are in the root dir of eCAV, and next let's install the dependencies. <strong>We highly
...
. setup.sh
```
If everything works correctly, you will see a cache folder is created in your eCAV root dir, and the terminal shows
...
This section is only needed for the users who want to test perception algorithms. By default, eCAV does not require
pytorch installed and it retrieves the object positions from the server directly. Once perception module is activated,
then eCAV will use yolov5 with pytorch to run object detection. <br>
...
The command belows shows an example of installing pytorch v1.8.0 with cuda 11.1 in ecav
environment.
...
If you want to use OpenScenario to conduct scenario testing, e.g. `python ecav.py -t openscenario_carla -v 0.9.12`, you need to install OpenScenario first. <br>

Please follow the [openscenario installation](coperception/setup_scenario_runner.md) to install OpenScenario.

## Docker Installation
eCAV provides docker image for users to run directly.
...
- `CARLA_VERSION`: Change to the version you want. Notice after(including) eCAV 0.1.4, only CARLA >= 0.9.14 will be supported.
...
- `OPENCDA_FULL_INSTALL`: If set true, it will run the `setup.sh` in eCAV automatically during building up the docker to install carla api into the python environment.
...
docker run -it --rm \
  ...
  --name ecav_container ecav_docker /bin/bash
```
