...
- [x] **`collision_time_ahead: 2 → 4` (run-10 finding):** at `collision_time_ahead=2`, hazard fires when TTC < 2 s — with ego at 11 m/s that is only ~22 m from the ambulance. Ego brakes over ~12 m and stops at x≈269, only ~5 m from the ambulance rear. When `do_overtake` fires, the merge path advances ego forward and right simultaneously; from 5 m back the ego front clips the ambulance corner before the lateral shift clears it. Raising to 4 s fires hazard at ~44 m out, stopping ego around x≈245 (~30 m clearance) — enough run-up for the full lane-change to complete before the obstacle.
- [ ] **OPEN — `behavior_agent.py:1555` curved-road suppression bug (Tyler scope).** `run_step` step-8:
  ```python
  ...
  ```
  The `potential_curved_road` term forces car-following — suppressing the overtake path — even when `overtake_allowed=True` and `overtake_counter==0`. Observed in run 10: when ego begins the right-merge to clear the ambulance, the merge waypoints look curved to the local planner → `potential_curved_road=True` → step-8 fires → ego stalls mid-merge at y=138.4 instead of completing the shift to lane -4 (y≈139.6). Tyler flagged this himself with `#TL - Why is this logic here?` at that line. Fix: remove the `potential_curved_road` term from the step-8 guard. Not in our scope; bring to Tyler sync.
