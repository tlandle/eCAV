...
APPROACHES = {
    "south": {
        ...
        "destination_straight": (-84.8, 200.0, 0.5), # continue north
    },
    "west": {
...
def distribute_egos(n_egos, stagger_m=20.0):
    ...
    Distribution strategy:
      ...
      - First ego on south approach always gets left_turn intention
    ...
    for i in range(n_egos):
        ...
        # Intention: first ego on south always turns left, others go straight
        if approach_name == "south" and idx == 0:
            intention = "left_turn"
...
def main():
    ...
    n = args.num_egos
    egos = distribute_egos(n, stagger_m=args.stagger)
    ...
    yaml_path = CONFIG_DIR / f"openscenario_3_edge_late_fusion_{n}ego.yaml"
    run_path = SCENARIO_DIR / f"openscenario_3_edge_late_fusion_{n}ego.py"

    xml_path.write_text(generate_xml(egos, n))
    py_path.write_text(generate_viz_py(egos, n))
    yaml_path.write_text(generate_yaml(egos, n))
