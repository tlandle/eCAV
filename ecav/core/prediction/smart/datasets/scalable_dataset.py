...
class MultiDataset(Dataset):
    ...
    def get(self, idx):
        while True:                                  # loop until good scene
            with open(self.raw_paths[idx], "rb") as f:
                data = pickle.load(f)
            try:
                return self.token_processor.preprocess(data)
            except KeyError as e:
                if str(e) == "'agent'":               # missing focal track
                    idx = (idx + 1) % len(self.raw_paths)   # skip ahead
                else:
                    raise
