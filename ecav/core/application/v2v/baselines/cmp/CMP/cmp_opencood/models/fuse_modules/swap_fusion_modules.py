...
from cmp_opencood.models.sub_modules.base_transformer import \
...
class SwapFusionBlockMask(nn.Module):
    ...
    This class is a specific type of processing block that includes window attention and grid attention mechanisms, with support for masking to enable multi-vehicle cooperation.
    The class uses PreNormResidual and FeedForward layers, which are common in Transformer architectures, indicating that it likely employs a variant of the Transformer model adapted for spatial data.
    The attention mechanisms are applied in two stages: window attention followed by grid attention, with each stage consisting of an attention layer and a feedforward layer.
    """
...
class SwapFusionBlock(nn.Module):
    ...
    Similar to SwapFusionBlockMask, this class also contains window and grid attention mechanisms.
    The primary difference seems to be the absence of mask handling, which implies this version might be used in contexts where masking is not required.
    The structure and processing flow are similar, with the data undergoing window attention and grid attention in sequence.
    """
...
class SwapFusionEncoder(nn.Module):
    ...
    This class appears to be an encoder that uses multiple layers of SwapFusionBlock or SwapFusionBlockMask.
    It is configurable in terms of depth, types of layers, and dimensions, allowing it to adapt to different input sizes and requirements.
    The class also includes an MLP (multi-layer perceptron) head at the end, which is typical for producing final encoded representations from Transformer-like models.
    The forward pass sequentially applies each stage in self.layers to the input, followed by processing through the MLP head.
    """
