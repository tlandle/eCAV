...
from cmp_opencood.models.sub_modules.torch_transformation_utils import \
    get_discretized_transformation_matrix, get_transformation_matrix, \
    warp_affine, get_rotated_roi
from cmp_opencood.models.sub_modules.convgru import ConvGRU
...
class V2VNetFusion(nn.Module):
    ...
    def forward(self, x, record_len, pairwise_t_matrix):
        ...
        # print("split_x: ", split_x[0].shape)
        # print("split_x: ", split_x[1].shape)
        # print("split_x: ", split_x[2].shape)
        # print("split_x: ", split_x[3].shape)
        # (B,L,L,2,3)
        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                if batch_node_features[b].shape[0] < N:
                    N = batch_node_features[b].shape[0]
                    t_matrix = t_matrix[:N,:,:,:]
                ...
                for i in range(N):
                    ...
                    # print(current_t_matrix.shape)

                    # print("current_t_matrix: ", current_t_matrix.shape)
                    # print("batch_node_features[b]: ", batch_node_features[b].shape)

                    # batch_node_features_this = batch_node_features[b]
                    # if batch_node_features_this.shape[0] < current_t_matrix.shape[0]:
                    #     current_t_matrix = current_t_matrix[:batch_node_features_this.shape[0],:,:]
                    #     N = batch_node_features_this.shape[0]
                    # else:
                    #     batch_node_features_this = batch_node_features_this[:current_t_matrix.shape[0],:,:,:]
                    #     N = current_t_matrix.shape[0]
                    ...
                    ego_agent_feature = batch_node_features[b][i].unsqueeze(
                        0).repeat(N, 1, 1, 1)
                    #(N,2C,H,W)
                    # if ego_agent_feature.shape[0] != neighbor_feature.shape[0]:
                    # print("EGO: ", ego_agent_feature.shape)
                    # print("NEIGHBOR: ", neighbor_feature.shape)
                    # if ego_agent_feature.shape[0] < neighbor_feature.shape[0]:
                    #     neighbor_feature = neighbor_feature[:ego_agent_feature.shape[0],:,:,:]
                    # else:
                    #     ego_agent_feature = ego_agent_feature[:neighbor_feature.shape[0],:,:,:]
                    try:
