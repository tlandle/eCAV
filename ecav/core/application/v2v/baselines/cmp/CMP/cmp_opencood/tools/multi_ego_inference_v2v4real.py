...
from cmp_opencood.utils.common_utils import torch_tensor_to_numpy
import cmp_opencood.hypes_yaml.yaml_utils as yaml_utils
from cmp_opencood.tools import train_utils, inference_utils
from cmp_opencood.data_utils.opv2v.datasets import build_opv2v_dataset
from cmp_opencood.data_utils.v2v4real.datasets import build_v2v4real_dataset
...
from cmp_opencood.utils import eval_utils
from cmp_opencood.utils.box_utils_v2v4real import corner_to_standup_box_torch, corner_to_center
from cmp_opencood.utils.box_utils import inverse_project_world_objects
from cmp_opencood.visualization import vis_utils
...
dataset_path = '/data1/Datasets/V2V4Real/'
...
def TrackByAB3DMOT(detection_result_for_tracking,
    ...
    for scenario_name, scenario_to_ego_cav_dict in tqdm(detection_result_for_tracking.items()):
        for ego_cav_id, ego_cav_to_timestamp_dict in scenario_to_ego_cav_dict.items():
            ...
            tracker = AB3DMOT(tracker_config, 'Car', calib=None, oxts=None,
            ...
            for timestamp_idx, (frame_result, ego_lidar_pose, transformation_matrix) in enumerate(zip(results_list, ego_lidar_pose_list, transformation_matrix_list)):
                ...
                for detection_item in frame_result:
                    ...
                    car_state = [x, y, z, l, w, h, np.rad2deg(theta), 0,0, confidence, 1]  # placeholder for v_x and v_y, update later, the last 1 means valid
                    
                    if ego_cav_id == 1 and not no_coop and dataset_name == 'v2v4real':
                        car_state = project_to_unified_coord(car_state, transformation_matrix[0])
            ...
            if no_coop and ego_cav_id == 1:
                for obj_id, states in data.items():
                    if obj_id < 0:
                        continue
                    for i in range(len(timestamp_keys)):
                        state = states[i]
                        if state[-1] != 0:
                            t_matrix = get_transformation_matrix(scenario_name, i)
                            state = project_to_unified_coord(state, t_matrix)
                            data[obj_id][i] = state
...
def get_transformation_matrix(scenario_name, i):

    ego_yaml = os.path.join(dataset_path, scenario_name, '0', '{0:06}.yaml'.format(i))
    cav_yaml = os.path.join(dataset_path, scenario_name, '1', '{0:06}.yaml'.format(i))
    
    if not os.path.exists(ego_yaml) or not os.path.exists(cav_yaml):
        return np.eye(4)
        
    ego_lidar_pose = yaml_utils.load_yaml(ego_yaml)['lidar_pose']
    cav_lidar_pose = yaml_utils.load_yaml(cav_yaml)['lidar_pose']

    transformation_matrix = x1_to_x2(cav_lidar_pose, ego_lidar_pose)

    return transformation_matrix

def project_to_unified_coord(car_state, transformation_matrix, is_detection=False):

    invalid_bbx_lidar = False
    object_pose = [car_state[0], car_state[1], car_state[2], 0, car_state[6], 0]
    extent = [car_state[3]/2, car_state[4]/2, car_state[5]/2]
    ...
    if invalid_bbx_lidar:
        ...
        if is_detection:
            return [bbx_lidar[0], bbx_lidar[1], bbx_lidar[2], bbx_lidar[5], bbx_lidar[4], bbx_lidar[3], bbx_lidar[6]]
        return [bbx_lidar[0], bbx_lidar[1], bbx_lidar[2], bbx_lidar[5], bbx_lidar[4], bbx_lidar[3], np.rad2deg(bbx_lidar[6]), 0, 0, car_state[9], 1]
