...
import cmp_opencood.utils.common_utils as common_utils
...
def corners_3d_to_boxes(corners3d):
    """
    Parameters
    ----------
    corners3d: torch.Tensor
        (N, 8, 3), the 8 corners of the bounding box.

    Returns:
    --------
    boxes3d: torch.Tensor
        (N, 7) [x, y, z, h, w, l, theta], (x, y, z) is the box center.
    """
    # Compute the center of the bounding box
    center = torch.mean(corners3d, dim=1)

    # Compute the dimensions of the bounding box
    l = torch.norm(corners3d[:, 0, :] - corners3d[:, 1, :], dim=1)
    w = torch.norm(corners3d[:, 1, :] - corners3d[:, 2, :], dim=1)
    h = torch.norm(corners3d[:, 0, :] - corners3d[:, 4, :], dim=1)

    # Compute the direction vector from corner 0 to corner 1
    direction = (corners3d[:, 1, :] - corners3d[:, 0, :]).clone()
    direction /= torch.norm(direction, dim=1, keepdim=True)

    # Compute the angle theta
    theta = torch.atan2(direction[:, 1], direction[:, 0])

    # Combine the results into the final bounding box format
    boxes3d = torch.stack([center[:, 0], center[:, 1], center[:, 2], h, l, w, theta], dim=1)
...
def corner_to_standup_box_torch(box_corner):
    """
    Process bounding boxes in 3D space and convert them into 2D axis-aligned bounding boxes
...
def get_mask_for_boxes_within_range_torch(boxes):
    ...
    """
    from cmp_opencood.data_utils.opv2v.datasets import GT_RANGE
...
def inverse_project_world_objects(output_dict, lidar_pose, order):
    """
    Inverse the projection of the objects from lidar coordinates to world coordinates.
    Parameters
    ----------
    output_dict : dict
        key: object id, value: object bbx (xyzlwhyaw).
    ...
    """
    object_dict = {}
    for object_id, bbx_lidar in output_dict.items():
        # Convert center representation to 8 corners
        corners_lidar = center_to_corner(bbx_lidar, order=order)
        lidar_yaw = bbx_lidar[0, 6]
        # Transform the 8 corners from lidar to world coordinates
        lidar_to_world_matrix = x_to_world(lidar_pose)
        corners_world = np.dot(lidar_to_world_matrix, np.r_[corners_lidar.T, [np.ones(corners_lidar.shape[0])]]).T[:, :3]
        # Calculate the original pose of the object in the world coordinate system
        xyz = np.mean(corners_world[[0, 3, 5, 6], :], axis=0)
        h = np.abs(np.mean(corners_world[4:, 2] - corners_world[:4, 2]))
        l = np.mean([np.linalg.norm(corners_world[0, :2] - corners_world[3, :2]),
                     np.linalg.norm(corners_world[2, :2] - corners_world[1, :2]),
                     np.linalg.norm(corners_world[4, :2] - corners_world[7, :2]),
                     np.linalg.norm(corners_world[5, :2] - corners_world[6, :2])])
        w = np.mean([np.linalg.norm(corners_world[0, :2] - corners_world[1, :2]),
                     np.linalg.norm(corners_world[2, :2] - corners_world[3, :2]),
                     np.linalg.norm(corners_world[4, :2] - corners_world[5, :2]),
                     np.linalg.norm(corners_world[6, :2] - corners_world[7, :2])])

        yaw = np.mean([np.arctan2(corners_world[1, 1] - corners_world[2, 1],
                                  corners_world[1, 0] - corners_world[2, 0]),
                       np.arctan2(corners_world[0, 1] - corners_world[3, 1],
                                  corners_world[0, 0] - corners_world[3, 0]),
                       np.arctan2(corners_world[5, 1] - corners_world[6, 1],
                                  corners_world[5, 0] - corners_world[6, 0]),
                       np.arctan2(corners_world[4, 1] - corners_world[7, 1],
                                  corners_world[4, 0] - corners_world[7, 0])])
        yaw = math.degrees(yaw)
        # Extract the original object's properties
        # location = (xyz - np.array([0, 0, h/2])).tolist()  # Assuming center is at the bottom of the bounding box
        location = [xyz[0], xyz[1], xyz[2]]
        center = np.array([0, 0, h/2])  # Assuming center is at the bottom of the bounding box
        extent = [l / 2, w / 2, h / 2]
        angle = np.array([0, yaw, 0])  # Assuming roll and pitch are 0
        # Create the original object dictionary
        object_dict[object_id] = [*location, *extent, yaw] #x, y, z, l, w, h, yaw
    return object_dict

def center_to_corner(center3d, order='lwh'):
    """
    Convert x, y, z, dx, dy, dz, yaw to 8 corners.
    Parameters
    ----------
    center3d : np.ndarray
        (N, 7)
    ...
    if center3d.ndim == 2:
        center3d = center3d[0]
    if order == 'lwh':
        l, w, h = center3d[3], center3d[4], center3d[5]
    elif order == 'hwl':
        l, w, h = center3d[5], center3d[4], center3d[3]
    else:
        sys.exit('Unknown order')
    theta = center3d[6]
    cos_t = np.cos(theta)
    ...
    rot_mat = np.array([[cos_t, -sin_t, 0],
                        [sin_t,  cos_t, 0],
                        [0,      0,     1]])
    # Create a bounding box in local coordinates
    ...
    corners += np.array(center3d[:3]).reshape(3, 1)
    return corners.T
