...
class AB3DMOT(object):			  	
	...
	def ego_motion_compensation(self, frame, trks):
		...
		for index in range(len(self.trackers)):
			...
			compensated = xyz #need to check whether egomotion_compensation_ID is necessary
	...
	def birth(self, dets, info, unmatched_dets, cav_ids):
		# create and initialise new trackers for unmatched detections

		# dets = copy.copy(dets)
		new_id_list = list()					# new ID generated for unmatched detections
		for i in unmatched_dets:        			# a scalar of index
			trk = KF(Box3D.bbox2array(dets[i]), info[i, :], self.ID_count[0], cav_ids[i])
	...
	def output(self):
		...
		for trk in reversed(self.trackers):
			...
			if ((trk.time_since_update < self.max_age) and (trk.hits >= self.min_hits or self.frame_count <= self.min_hits)):      
				results.append(np.concatenate((d, [trk.id], trk.info, [trk.cav_id])).reshape(1, -1)) 		
			num_trks -= 1
	...
	def track(self, dets_all, frame, seq_name):
		...
		"""
		dets, info, cav_ids = dets_all['dets'], dets_all['info'], dets_all['cav_id']         # dets: N x 7, float numpy array
		...
		# create and initialise new trackers for unmatched detections
		new_id_list = self.birth(dets, info, unmatched_dets, cav_ids)

		# output existing valid tracks
		results = self.output()
		if len(results) > 0: results = [np.concatenate(results)]		# h,w,l,x,y,z,theta, ID, other info, confidence, cav_id
