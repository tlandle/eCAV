...
def combine_trk_cat(split, dataset, method, suffix, num_hypo):
	...
	set_split = 'testset' if split == 'test' else 'trainset'
	root_dir = os.path.join(file_path, '../../results', dataset, set_split)
	...
	# source directory
	subset = ['%s_%s_%s_%s' % (method, cat, 'test', suffix) for cat in cfg.cat_list]
