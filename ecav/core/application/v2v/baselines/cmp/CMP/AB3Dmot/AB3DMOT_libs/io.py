...
def load_6dof_oxts(file):
	# load from raw file
	with warnings.catch_warnings():
		warnings.simplefilter("ignore")
		poses = np.loadtxt(file, delimiter=',') 	# load detections, N x 7
	return poses[:, 1:]

def get_frame_det(dets_all, frame):
	
	# get irrelevant information associated with an object, not used for associationg
	ori_array = dets_all[dets_all[:, 0] == frame, 14].reshape((-1, 1))		# orientation
	other_array = dets_all[dets_all[:, 0] == frame, 1:7] 					# other information, e.g, 2D box, ...
	additional_info = np.concatenate((ori_array, other_array), axis=1)		

	# get 3D box
	dets = dets_all[dets_all[:, 0] == frame, 7:14]		
	cav_id = dets_all[dets_all[:, 0] == frame, 15]
	vel = dets_all[dets_all[:, 0] == frame, 16:18]

	dets_frame = {'dets': dets, 'info': additional_info, 'cav_id': cav_id, 'vel': vel}
	return dets_frame
...
def get_saving_dir(eval_dir_dict, seq_name, save_dir, num_hypo, ego_cav_id):

	# create dir and file for saving
	eval_file_dict, save_trk_dir = dict(), dict()
	for index in range(num_hypo):
		eval_file_dict[index] = os.path.join(eval_dir_dict[index], seq_name + f'-{ego_cav_id}.txt')
		eval_file_dict[index] = open(eval_file_dict[index], 'a')
...
def save_results(res, save_trk_file, eval_file, det_id2str, frame, score_threshold):
	...
	bbox3d_tmp, id_tmp, ori_tmp, type_tmp, bbox2d_tmp_trk, conf_tmp, cav_id = \
		res[0:7], res[7], res[8], det_id2str[res[9]], res[10:14], res[14], res[15]
	 
	# save in detection format with track ID, can be used for dection evaluation and tracking visualization
	str_to_srite = '%s -1 -1 %f %f %f %f %f %f %f %f %f %f %f %f %f %d %d\n' % (type_tmp, ori_tmp,
		bbox2d_tmp_trk[0], bbox2d_tmp_trk[1], bbox2d_tmp_trk[2], bbox2d_tmp_trk[3], 
		bbox3d_tmp[0], bbox3d_tmp[1], bbox3d_tmp[2], bbox3d_tmp[3], bbox3d_tmp[4], bbox3d_tmp[5], bbox3d_tmp[6], conf_tmp, id_tmp, cav_id)
