...
OPV2V_SEQMAP_FILE_NAME = 'opv2v_val_evaluate_tracking.seqmap.val'
V2V4Real_SEQMAP_FILE_NAME = 'v2v4real_val_evaluate_tracking.seqmap.val'

num_sample_pts = 41.0

def parse_arguments():
    parser = argparse.ArgumentParser(description='tracking evaluation configuration')
    ...
    parser.add_argument('--dataset', type=str, required=True, 
                        help='dataset name: opv2v or v2v4real (required)')

    args = parser.parse_args()
    return args

def save_traj_to_kitti(ab3dmot_tracking_path, kitti_tracking_save_path, is_gt=False, dataset_name='opv2v'):

    if is_gt and os.path.exists(kitti_tracking_save_path):
        print("Already generate KITTI tracking label.")
        return 
    if not os.path.exists(kitti_tracking_save_path):
        os.makedirs(kitti_tracking_save_path)

    for track_file in os.listdir(ab3dmot_tracking_path):
        if track_file.endswith('pickle'):
            if dataset_name == 'opv2v':
                scenario_name = track_file.split('-')[0]
            elif dataset_name == 'v2v4real':
                filename_no_ext = os.path.splitext(track_file)[0]
                scenario_name = '-'.join(filename_no_ext.split('-')[:-2])
            ego_cav_id = track_file.split('-')[-2]
            if dataset_name == 'opv2v':
                kitti_tracking_save_file = os.path.join(kitti_tracking_save_path, f'{scenario_name}-{ego_cav_id}.txt')
            elif dataset_name == 'v2v4real':
                kitti_tracking_save_file = os.path.join(kitti_tracking_save_path, f'{scenario_name}-{ego_cav_id}.txt')

            with open(os.path.join(ab3dmot_tracking_path, track_file), 'rb') as track_pickle:
                track = pickle.load(track_pickle) # cav_id : sequence x states, n x (x, y, z, l, w, h, theta, valid)
                n_frames = len(track['timestamps'])
                with open(kitti_tracking_save_file, 'w') as f:
                    for timestamp in range(n_frames):
                        for cav_id, states in track['data'].items():
                            if cav_id < 0:
                                continue
                            if timestamp >= len(states):
                                print("invalid timestamp: ", timestamp)
                                continue
                            if track['data'][cav_id][timestamp] == [0, 0, 0, 0, 0, 0, 0, 0]:
                                # print("empty states")
                                continue
                            if track['data'][cav_id][timestamp][-1] == 0:
                                continue
                            else:
                                state = track['data'][cav_id][timestamp]
                                # h, w, l, x, z, y, theta
                                box_3d = ' '.join([str(state[5]), str(state[4]), str(state[3]), str(state[0]), str(state[2]), str(state[1]), str(np.radians(state[6]))])
                                if dataset_name == 'opv2v':
                                    box_3d = ' '.join([str(2*state[5]), str(2*state[4]), str(2*state[3]), str(state[0]), str(state[2]), str(state[1]), str(np.radians(state[6]))])
                                ab3dmot_tracking_label_string = '%d %d %s 0 0 0 0 0 0 0 %s\n' % (
                                    timestamp, cav_id, 'Car', box_3d
                                )
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path=None, t_path=None, max_truncation = 0, min_height = 25, max_occlusion = 2, \
        mail=None, cls="car", eval_3diou=True, eval_2diou=False, num_hypo=1, thres=None, project_dir=None, dataset=None):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        if dataset == 'opv2v':
            filename_test_mapping = os.path.join(project_dir, 'AB3Dmot/scripts/KITTI' , OPV2V_SEQMAP_FILE_NAME)
        elif dataset == 'v2v4real':
            filename_test_mapping = os.path.join(project_dir, 'AB3Dmot/scripts/KITTI' , V2V4Real_SEQMAP_FILE_NAME)
        ...
        with open(filename_test_mapping, "r") as fh:
            for i,l in enumerate(fh):
                ...
                self.n_frames.append(int(fields[3]) - int(fields[2]))
        fh.close()
        self.n_sequences = i+1
        print('self.n_sequences: ', self.n_sequences) # 
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            if self.dataset == 'opv2v':
                filename       = os.path.join(root_dir, f"{s_name}-{ego_ids['testset'][s_name]}.txt")
            elif self.dataset == 'v2v4real':
                filename       = os.path.join(root_dir, f"{s_name}-0.txt")
            f              = open(filename, "r")
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres,path_to_kitti_tracking_gt,path_to_kitti_tracking_results,project_dir,dataset):
    """
    ...
    for c in class_list:
        e = trackingEvaluation(t_sha=result_sha, gt_path=path_to_kitti_tracking_gt, t_path=path_to_kitti_tracking_results, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou,num_hypo=num_hypo,thres=thres,project_dir=project_dir, dataset=dataset)
...
if __name__ == "__main__":
    ...
    path_to_kitti_tracking_gt = os.path.join(project_dir, 'AB3Dmot/scripts/KITTI/', args.dataset + '_label/' , ('train' if args.use_train_set else 'test'))
    
    path_to_kitti_tracking_results = os.path.join(project_dir, 'preprocessed_data', args.dataset, f'new_ab3dmot_logs_{args.perception_model_name}', ('train' if args.use_train_set else 'test'), 'data_0')
    path_to_ab3dmot_tracking_results =  os.path.join(project_dir, 'preprocessed_data', args.dataset, 'tracking_trajs_' + args.perception_model_name, ('train' if args.use_train_set else 'test'))
    save_traj_to_kitti(path_to_ab3dmot_tracking_results, path_to_kitti_tracking_results, is_gt=False, dataset_name=args.dataset)

    result_sha = args.dataset +  ('/train/' if args.use_train_set else '/test/')  + args.perception_model_name
    result_save_path = os.path.join("./results/KITTI", result_sha)
    ...
    # evaluate results
    success = evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, path_to_kitti_tracking_gt, path_to_kitti_tracking_results, project_dir, args.dataset)